...
- **输出上限**：默认 `maxTokens: 1024`，避免长尾生成拖慢识别
- **灰度兜底**：没有视觉识别器时，可启用有边界的灰度 ASCII 回退，让纯文本主模型至少看到粗略布局
...
- id: image-recognition
  ...
  config:
    ...
    maxTokens: 1024          # 单次输出 token 上限；调低更快，调高可提取更长文本
    grayscaleEnabled: false  # true = 没有视觉识别器时回退为有边界的灰度 ASCII
    grayscaleSize: 32        # 回退网格宽/高
    prompt: >-               # 可选，默认中文描述+文字提取提示词
      你是图像识别助手。请用中文简洁描述图片内容（主体、布局、背景、风格）；
      逐字提取图中可见文字（界面文案、代码、表格），保持原有结构；
      界面截图或图表说明组成部分和含义；
      「密钥」(API key) 等筛选、统计或配置术语统一写作「供应商」(provider)。
      直接输出识别结果，不要客套。
```
